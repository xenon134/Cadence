...
class Slider(Frame):
    ...
    def __init__(
        ...
        Slider.LINE_COLOR = lineColor
        Slider.LINE_WIDTH = lineWidth
        Slider.BAR_COLOR_INNER = barColor
        Slider.BAR_COLOR_OUTTER = barColor
        Slider.BAR_RADIUS = barRadius
        Slider.BAR_RADIUS_INNER = 0

        Frame.__init__(self, master, height=height, width=width, bg=bg)
        self.master = master
        if init_lis == None:
            init_lis = [min_val]
        ...
        self.canv = Canvas(
            self, height=self.canv_H, width=self.canv_W, bg=bg, highlightthickness=0
        )
        self.canv.pack()
